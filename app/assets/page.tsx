...
export default function AssetsPage() {
  ...
  async function loadAssets() {
    ...
    try {
      const { data, error: err } = await supabase
        .from('assets')
        .select('id, category, sub_category, location, brand, type, serial_number, expired_date, status')
        .limit(500)
  ...
  useEffect(() => {
    if (filterCategory) {
      const subs = [...new Set(assets.filter(a => a.category === filterCategory).map(a => a.sub_category).filter(Boolean))]
  ...
  const availableTypes = filterSubCategory
    ? [...new Set(assets.filter(a => a.sub_category === filterSubCategory && a.type).map(a => a.type as string))]
    : []
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-main)' }}>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', padding: '24px', borderRadius: '8px', boxShadow: 'var(--shadow)', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 600, marginBottom: '16px', color: 'var(--primary)' }}>🔍 Search & Filter</h3>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <input
              ...
              style={{ flex: 1, minWidth: '200px', padding: '10px 14px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '14px' }}
            />
            <select
        ...
        {loading ? (
          <div style={{ background: 'var(--bg-card)', padding: '48px', borderRadius: '8px', textAlign: 'center', color: 'var(--text-secondary)' }}>
            Loading assets...
          </div>
        ) : (
          <div style={{ background: 'var(--bg-card)', borderRadius: '8px', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {filtered.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ borderBottom: '1px solid var(--border-light)', background: i % 2 === 0 ? 'white' : 'var(--bg-main)', transition: 'background 0.2s' }}
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 16px', fontSize: '12px', color: 'var(--text-secondary)' }}>{a.brand || '—'}</td>
                      <td style={{ padding: '14px 16px', fontSize: '12px', color: 'var(--text-secondary)' }}>{a.type || '—'}</td>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <span style={{ display: 'inline-block', padding: '4px 10px', borderRadius: '4px', background: statusInfo.bg, color: statusInfo.color, fontSize: '12px', fontWeight: 600 }}>
                          {statusInfo.label}
                        </span>
                      </td>
                      <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                        <button onClick={() => openEdit(a)} style={{ padding: '6px 12px', background: 'var(--secondary)', color: 'var(--primary)', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}
            ...
            {filtered.length === 0 && (
              <div style={{ padding: '48px', textAlign: 'center', color: 'var(--text-secondary)' }}>No assets found</div>
            )}
          </div>
        )}
      </div>
      ...
      {editId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--bg-card)', padding: '32px', borderRadius: '8px', maxWidth: '500px', width: '90%', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--primary)', marginBottom: '24px' }}>Edit Asset: {editId}</h2>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--primary)' }}>Status</label>
              <select value={editStatus} onChange={(e) => setEditStatus(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '14px', background: 'white' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--primary)' }}>Expired Date</label>
              <input type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--border)', borderRadius: '6px', fontSize: '14px' }} />
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => setEditId(null)} disabled={saving} style={{ flex: 1, padding: '12px', background: 'var(--text-secondary)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: 600, opacity: saving ? 0.6 : 1 }}>Cancel</button>
              <button onClick={() => handleSave(editId)} disabled={saving} style={{ flex: 1, padding: '12px', background: 'var(--secondary)', color: 'var(--primary)', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: 600, opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving...' : '✓ Save'}</button>
            </div>
          </div>
        </div>
      )}
